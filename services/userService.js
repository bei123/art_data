...
const { loadDeliveryUnitsByOrderItemIds } = require('./digitalQrDeliveryService');
...
function mapPurchasedProductItem(item) {
  ...
  if (item.type === 'right') {
    ...
    const units = Array.isArray(item.delivery_units) ? item.delivery_units : [];
    const filled = units.filter((u) => u.qr_code_url);
    const qrCodeUrl = filled[0]?.qr_code_url || item.delivery_qr_code_url || null;
    product = {
      ...product,
      digital_artwork_id:
      ...
      qr_code_uploaded_at: filled[0]?.delivered_at || item.delivery_qr_code_at || null,
...
async function getPurchasedProducts(userId, query = {}) {
  try {
    ...
    const digitalIds = (items || [])
      ...
      .map((row) => row.id);
    const unitsByItemId = await loadDeliveryUnitsByOrderItemIds(digitalIds);
