...
const WECHAT_BANK_TYPE_MAP = {
  ...
  "JCB_DEBIT": "晋城银行（借记卡）",
  "YNMLBY_DEBIT": "云南马龙北银村镇银行（借记卡）",
  "JCBK_CREDIT": "晋城银行（信用卡）",
  "SPBY_DEBIT": "云南石屏北银村镇银行（借记卡）",
  "JDHDB_DEBIT": "上海嘉定洪都村镇银行（借记卡）",
  "XSBYCZ_DEBIT": "云南西山北银村镇银行（借记卡）",
  "JDZCCB_DEBIT": "景德镇市商业银行（借记卡）",
  "XPBYCZYH_DEBIT": "云南新平北银村镇银行（借记卡）",
  "JHCCB_CREDIT": "金华银行（信用卡）",
  "YJBY_DEBIT": "元江北银村镇银行（借记卡）",
  "JHCCB_DEBIT": "金华银行（借记卡）",
  "XSBY_DEBIT": "重庆秀山北银村镇银行（借记卡）",
  "MYB_ECNY": "网商银行(支付宝)数字人民币钱包",
  "YCBY_DEBIT": "重庆永川北银村镇银行（借记卡）",
  "YHWJ_DEBIT": "邮惠万家银行（借记卡）",
  "DINERSCLUB_CREDIT": "DINERSCLUB",
  "BOSZS_CREDIT": "石嘴山银行（信用卡）",
  "MASTERCARD_CREDIT": "MASTERCARD",
  "ZLLFYH_DEBIT": "涿鹿利丰村镇银行（借记卡）",
  "VISA_CREDIT": "VISA",
  "HDCB_CREDIT": "邯郸银行（信用卡）",
  "AMERICANEXPRESS_CREDIT": "AMERICANEXPRESS",
  "LZBY_DEBIT": "河北蠡州北银农商银行（借记卡）",
  "DISCOVER_CREDIT": "DISCOVER",
  "NABY_DEBIT": "农安北银村镇银行（借记卡）",
  "UPQUICKPASS_CREDIT": "云闪付（信用卡）",
  "JCB_CREDIT": "JCB",
  "GWTHP_DEBIT": "Hipay（借记卡）",
  "GWTWPHK_DEBIT": "微信香港钱包（借记卡）",
  "GWTZLP_DEBIT": "Zalopay（借记卡）",
  "GWTCY_DEBIT": "Changipay（借记卡）",
  "BANK_DEBIT": "借记卡",
  "BANK_CREDIT": "信用卡",
  "BANK_ECNY": "数字人民币钱包",
  "OTHERS": "其他（银行卡以外）"
...
const GENERIC_BANK_TYPE_CODES = new Set(['BANK_DEBIT', 'BANK_CREDIT', 'BANK_ECNY', 'OTHERS'])

const GENERIC_BANK_TYPE_NAMES = new Set(['借记卡', '信用卡', '数字人民币钱包', '其他（银行卡以外）'])

function looksLikeBankTypeCode(value) {
	if (value == null || value === '') return false
	return /^[A-Z0-9_]+$/i.test(String(value).trim())
}

function isGenericBankName(name) {
	if (name == null || name === '') return true
	return GENERIC_BANK_TYPE_NAMES.has(String(name).trim())
}

function lookupBankTypeName(code) {
	if (code == null || code === '') return ''
	const key = String(code).trim().toUpperCase()
	return WECHAT_BANK_TYPE_MAP[key] || ''
}

function getWechatBankTypeLabel(code, fallbackDisplay) {
	const rawCode = code != null && code !== '' ? String(code).trim() : ''
	const rawFallback = fallbackDisplay != null && fallbackDisplay !== '' ? String(fallbackDisplay).trim() : ''

	if (rawFallback && looksLikeBankTypeCode(rawFallback)) {
		const fromFallbackCode = lookupBankTypeName(rawFallback)
		if (fromFallbackCode && !isGenericBankName(fromFallbackCode)) {
			return fromFallbackCode
		}
	}

	if (!rawCode) {
		if (rawFallback && looksLikeBankTypeCode(rawFallback)) {
			return lookupBankTypeName(rawFallback) || rawFallback
		}
		return rawFallback
	}

	const mapped = lookupBankTypeName(rawCode)
	const codeKey = rawCode.toUpperCase()

	if (mapped) {
		const mappedIsGeneric = GENERIC_BANK_TYPE_CODES.has(codeKey) || isGenericBankName(mapped)
		if (mappedIsGeneric) {
			if (rawFallback && looksLikeBankTypeCode(rawFallback)) {
				const alt = lookupBankTypeName(rawFallback)
				if (alt && !isGenericBankName(alt)) return alt
			}
			if (rawFallback && !looksLikeBankTypeCode(rawFallback) && !isGenericBankName(rawFallback)) {
				return rawFallback
			}
		}
		return mapped
...
/** 从订单 payment / 顶层字段解析付款银行展示名 */
function resolveOrderPaymentBankLabel(order) {
	if (!order || typeof order !== 'object') return ''
	const pay = order.payment || order.pay_info || order.payment_display || {}
	const bankCode =
		pay.bank_type ||
		pay.bankType ||
		order.bank_type ||
		order.bankType ||
		(order.order && order.order.bank_type) ||
		''
	const bankDisplay =
		pay.bank_type_display ||
		pay.bank_type_label ||
		pay.bank_name ||
		pay.bankName ||
		order.bank_type_display ||
		order.bank_type_label ||
		(order.order && order.order.bank_type_display) ||
		''
	return getWechatBankTypeLabel(bankCode, bankDisplay)
}

function bankTypeDisplay(code) {
  if (!code) return null
  const key = String(code).trim().toUpperCase()
  if (WECHAT_BANK_TYPE_MAP[key]) return WECHAT_BANK_TYPE_MAP[key]
  if (key === 'OTHER') return WECHAT_BANK_TYPE_MAP.OTHERS || '其他（银行卡以外）'
  return String(code)
}

module.exports = {
  WECHAT_BANK_TYPE_MAP,
  bankTypeDisplay,
  getWechatBankTypeLabel,
  resolveOrderPaymentBankLabel,
  lookupBankTypeName,
}
