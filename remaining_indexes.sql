...
-- 复合索引：user_id + status (用于按用户和状态查询)
CREATE INDEX idx_real_name_registrations_user_status ON real_name_registrations(user_id, status);

-- 复合索引：status + created_at (用于按状态和时间排序)
CREATE INDEX idx_real_name_registrations_status_created ON real_name_registrations(status, created_at DESC);

-- 2. 为merchant_images表添加索引

-- 主要查询模式：WHERE merchant_id = ? (用于查询商家图片)
-- 这个索引已经存在，但我们需要确保它是最优的
-- CREATE INDEX merchant_id ON merchant_images(merchant_id);

-- 复合索引：merchant_id + id (用于按merchant_id和id排序)
CREATE INDEX idx_merchant_images_merchant_id_order ON merchant_images(merchant_id, id);

-- 复合索引：merchant_id + image_url (用于按merchant_id和图片URL查询)
CREATE INDEX idx_merchant_images_merchant_url ON merchant_images(merchant_id, image_url);
...
-- 主要查询模式：WHERE status = ? (用于按状态查询退款申请)
-- 这个索引已经存在，但我们需要确保它是最优的
